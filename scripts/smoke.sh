...
test_group_network_role_critic() {
  ...
  note "network: --role critic directive"

  out=$("$BIN" --no-stream --role critic --tools read,grep,ls "Audit src/main.zig for bugs"); rc=$?
  ...
  if printf '%s' "$out" | grep -qE '<APPROVED>|<BLOCKED>'; then
    ...
    ok "--role critic response contains <APPROVED> or <BLOCKED>" 1 0
